				     ...
				     struct wpa_driver_ap_params *params)
{
	...
	if (!msg)
		return -ENOMEM;
	wpa_printf(MSG_DEBUG, "access_network_type:%d\n\n\n\n\n",params->access_network_type);
...
{
	struct i802_bss *bss = priv;
	fprintf(stderr,"freq->freq=%d\n freq->ht_enabled=%d\n,freq->sec_channel_offset=%d\n",freq->freq, freq->ht_enabled,
...
fprintf(stderr,"aifs = %d\n",aifs);
fprintf(stderr,"cw_min = %d\n",cw_min);
fprintf(stderr,"cw_max = %d\n",cw_max);
fprintf(stderr,"burst_time = %d\n",burst_time);
...
static int i802_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr,
			   int reason)
{
	wpa_hexdump(MSG_DEBUG, "own_addr",own_addr, ETH_ALEN);
	wpa_hexdump(MSG_DEBUG, "addr",addr, ETH_ALEN);
	fprintf(stderr,"%d\n",reason);
	fprintf(stderr,"\n\n\n\n");
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
