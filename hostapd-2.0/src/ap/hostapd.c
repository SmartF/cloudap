...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
	...
	fprintf(stderr,"hostapd.c line 644");
        fprintf(stderr,"force_ifname = ");
	int m;
	for(m=0;m<IFNAMSIZ;m++)
	{
		fprintf(stderr,"%c,",force_ifname[m]);
	}
	fprintf(stderr,"\n");

        fprintf(stderr,"if_addr = ");
	for(m=0;m<ETH_ALEN;m++)
	{
		fprintf(stderr,"%c,",if_addr[m]);
	}
	fprintf(stderr,"\n");
	...
	for(m=0;m<=IFNAMSIZ;m++)
	{
		fprintf(stderr,"%c,",hapd->conf->bridge[m]);
	}
	fprintf(stderr,"\n");
	fprintf(stderr,"hapd->conf->bridge[0] = %c\n",hapd->conf->bridge[0]);
